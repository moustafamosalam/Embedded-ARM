...
u8 	 USART1_voidRcvChar(void)
{
	u8 Local_u8Data = 0;
	u32 Timeout=0;
	while((GET_BIT(USART1->SR,5) == 0) && (Timeout<4000000))
	{
		Timeout++;
	}
	if (Timeout == 4000000)
...
u8*	 USART1_voidRcvString(void)
{
	u8 i = 0;
	u8 Local_u8Data = 0;
	while( (( Local_u8Data = USART1_voidRcvChar() ) != 13) && ((Local_u8Data = USART1_voidRcvChar() ) != 255))
	{
		String[i] = Local_u8Data;
		i++;
	}
	String[i] = '\0';
	return (String);
